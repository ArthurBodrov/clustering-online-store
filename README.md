...
До фильтра было *42_448_764 rows* , после фильтрации стало *338_018 rows*. Отобрали из 5,28 GB данных 27 MB нужных нам. Двигаемся дальше.
...
`category_id` и `event_type` уже не понадобятся, удалим их.
...
# использую `.iloc[-1]` из-за упорядоченности данных
```
